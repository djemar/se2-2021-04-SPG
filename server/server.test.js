...
describe("Get products by dates API", () => {
  ...
  test("Get products between dates-category success", async () => {
    ...
    const response = await request
      .post("/api/products-between-dates-category")
      .send(body);
...
describe("Create product API", () => {
  test("Product added success", async () => {
    const product = {
      ...
      image_path:
        "https://m.media-amazon.com/images/I/61vRV720szS._AC_SX466_.jpg",
...
describe("Delete all pending cancellation orders API", () => {
  test("Delete all pending cancellation order API success", async () => {
    const order = {
      ref_user: 13,
      productList: [{ ref_product: 122, quantity: 1 }],
      date_order: "222",
    };
    ...
    const response3 = await request.get(
      "/api/delete-all-pending-cancellation-order/"
    );
